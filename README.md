...
| **M8.6e** | **性能深化** ⭐ | 4 步优化: Step 6 (60% sub) + Step 7 (10K 2-fold OOF) + Step 10 (first-stage 100 trees) + Step 14 (20K train, 100 trees, 500 chart) | **213.85s → 152.85s (-28.5%, -61s)**, AUC -0.004, CATE +0.122 | (HEAD) |

**总投入**: 37 个测试文件 / **396 个测试用例** (全跑 66.4s) / **569 行反欺诈代码 + 432 行公平性代码 + 489 行叙事代码 + 1100 行 M8.6 验证代码 + M8.6e 性能代码** / **19 张图表** / **3 份决策报告 + 公平性 + fraud + causal_narrative_v2 字段** / **19 份设计文档**
...
```bash
# 端到端 pipeline (16 步, 含 STEP 3.5 多表 + STEP 14 反欺诈 + STEP 15 公平性 + STEP 16 叙事, ~153 秒 CPU 热跑, M8.6e 后)
...
| 指标 | M2 单表 | M5 8 表 | M5+ (CPU 优化) | M6 (GPU+Optuna) | M7 (+反欺诈) | M8.6d-end (修复) | **M8.6e (性能深化, 当前)** | 累计提升 |
|------|------:|-----------:|-----------:|-----------:|-----------:|-----------:|-----------:|-----:|
| 特征数 | 30 | 265 | 216 | 211 | 211 | 213 | **213** | +183 |
| 3-fold CV AUC | 0.7503 | 0.7763 | 0.7756 | **0.7803** | **0.7803** | 0.7765 | **0.7727** | +0.022 |
| 测试集 AUC-ROC | 0.7547 | 0.7803 | 0.7803 | **0.7803** | **0.7803** | 0.7802 | **0.7759** | +0.021 |
| 测试集 F1 (default) | 0.0344 | 0.0770 | 0.0735 | **0.0735** | **0.0735** | 0.0740 | **0.0744** | +0.040 |
| ATE (`AMT_CREDIT` → `TARGET`) | +0.0092 | +0.0092 | +0.0092 | +0.0092 | +0.0092 | +0.0092 | +0.0092 | — |
| CATE 一致性 (3 方法) | 0.578 | 0.548 | 0.548 | 0.548 | 0.548 | 0.548 | **0.670** | +0.092 |
| 反驳验证 | 3/4 | 3/4 | 3/4 | 3/4 | 3/4 | 3/4 | **3/4** | — |
| 公平性 4 切片 | — | — | — | — | — | 4/4 WARNING | **4/4 WARNING** (min_group_size=100) | — |
| 决策报告 | 3 份 | 3 份 | 3 份 | 3 份 | + fraud 字段 | + fraud + fairness + narrative_v2 | **同 M8.6d-end** | — |
| 单元测试 | 85 / 1.34s | 98 / 1.44s | 101 / 1.46s | 108 / 7.69s | 133 / 8.0s | 396 / 86.3s | **396 / 66.4s** | +311 |
| Pipeline 端到端耗时 | 84.8s | 244.9s (冷) | 184.5s (热) | 184.5s (热) | 194.9s (热) | 213.85s (热) | **152.85s (热)** | +68s |
...
| 文档 | 用途 |
|------|------|
| [`PROGRESS.md`](PROGRESS.md) | 20 个里程碑详细记录 (M0-M7 + M8.1-M8.6 + M8.6d-end + M8.6e 性能深化) |
