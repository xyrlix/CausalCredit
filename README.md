...
| **M8.6d-end** | **验证修复 + 公平性小样本组过滤** ⭐ | 特征列去重 (EXT_SOURCE_* 与 secondary 重名) + PC 共线性列剔除 (|ρ|>0.98) + 公平性 `min_group_size=100` 过滤 + BENCHMARKS/PROGRESS 增补 | 3 新测试, 393→**396** | (HEAD) |

**总投入**: 37 个测试文件 / **396 个测试用例** (全跑 86.3s) / **569 行反欺诈代码 + 432 行公平性代码 + 489 行叙事代码 + 1100 行 M8.6 验证代码** / **19 张图表** / **3 份决策报告 + 公平性 + fraud + causal_narrative_v2 字段** / **19 份设计文档**
...
```bash
# 端到端 pipeline (16 步, 含 STEP 3.5 多表 + STEP 14 反欺诈 + STEP 15 公平性 + STEP 16 叙事, ~214 秒 CPU 热跑)
...
| 指标 | M2 单表 | M5 8 表 | M5+ (CPU 优化) | M6 (GPU+Optuna) | M7 (+反欺诈) | M8.6 验证深化 (当前) | 累计提升 |
|------|------:|-----------:|-----------:|-----------:|-----------:|-----------:|-----:|
| 特征数 | 30 | 265 | 216 | 211 | 211 | **213** | +183 |
| 3-fold CV AUC | 0.7503 | 0.7763 | 0.7756 | **0.7803** | **0.7803** | **0.7765** | +0.026 |
| 测试集 AUC-ROC | 0.7547 | 0.7803 | 0.7803 | **0.7803** | **0.7803** | **0.7802** | +0.026 |
| 测试集 F1 (default) | 0.0344 | 0.0770 | 0.0735 | **0.0735** | **0.0735** | **0.0740** | +0.040 |
| ATE (`AMT_CREDIT` → `TARGET`) | +0.0092 | +0.0092 | +0.0092 | +0.0092 | +0.0092 | +0.0092 | — |
| CATE 一致性 (3 方法) | 0.578 | 0.548 | 0.548 | 0.548 | 0.548 | **0.548** | -0.030 |
| 反驳验证 | 3/4 | 3/4 | 3/4 | 3/4 | 3/4 | **3/4** | — |
| 公平性 4 切片 | — | — | — | — | — | **4/4 WARNING** (min_group_size=100) | — |
| 决策报告 | 3 份 | 3 份 | 3 份 | 3 份 | + fraud 字段 | **+ fraud + fairness + narrative_v2 字段** | — |
| 单元测试 | 85 / 1.34s | 98 / 1.44s | 101 / 1.46s | 108 / 7.69s | 133 / 8.0s | **396 / 86.3s** | +311 |
| Pipeline 端到端耗时 | 84.8s | 244.9s (冷) | 184.5s (热) | 184.5s (热) | 194.9s (热) | **213.85s (热)** | +129s |
...
| 文档 | 用途 |
|------|------|
| [`PROGRESS.md`](PROGRESS.md) | 19 个里程碑详细记录 (M0-M7 + M8.1-M8.6 + M8.6d-end 验证修复) |
