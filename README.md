...
## 当前实测结果（Home Credit, 30 万行 + 5 张二级表, M5+ 优化后）

> 详细 per-step 耗时、ATE/CATE/Refutation 数值、决策报告样例见 [`BENCHMARKS.md`](BENCHMARKS.md)

| 指标 | M2 单表 | M5 8 表 (无优化) | M5+ 8 表 (CPU 优化, 热跑) | 累计提升 |
|------|------:|-----------:|-----------:|-----:|
| 特征数 | 30 | 265 | 216 (砍 49 0-gain) | +186 |
| 3-fold CV AUC | 0.7503 | 0.7763 | **0.7756** | +0.025 |
| 测试集 AUC-ROC | 0.7547 | 0.7803 | **0.7803** | +0.026 |
| 测试集 F1 (default) | 0.0344 | 0.0770 | **0.0735** | +0.039 |
| ATE（`AMT_CREDIT` → `TARGET`, DoWhy） | +0.0092 | +0.0092 | +0.0092 | — |
| CATE 一致性（3 方法 mean Spearman） | 0.578 | 0.548 | 0.548 | -0.030 |
| 反驳验证 | 4 中 3 | 4 中 3 | 4 中 3 | — |
| 决策报告多样性 | 3 份 | 3 份 | 3 份 | — |
| 单元测试 | 85 / 1.34s | 98 / 1.44s | **101 / 1.46s** | +16 |
| Pipeline 端到端耗时 | 84.8s | 244.9s (冷) | **184.5s (热)** | +99.7s (净增) |

> M5+ 通过**多表聚合缓存** (65s→2s) + **L1 特征预筛选** (-16% 训练耗时) 把热跑耗时砍 25%, AUC 完全持平。
